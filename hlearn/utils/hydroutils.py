...
def make_MXS_labels (
    ...
    **kws
    ...
    return_obj: :class:`hlearn.utils.box.Boxspace`
    ...
    Returns 
    ---------
    MXS: array-like 1d or :class:`~hlearn.utils.box.Boxspace`
    ...
    >>> from hlearn.utils.hydroutils import classify_k, make_MXS_labels
...
>>> from hlearn.utils import naive_imputer, read_data, reshape 
>>> from hlearn.datasets import load_hlogs 
>>> from hlearn.utils.hydroutils import classify_k, find_aquifer_groups 
...
def label_importance (
    ...
    *, 
    ...
    -----------
    >>> from hlearn.datasets import load_hlogs
    >>> from hlearn.utils.hydroutils import label_importance, classify_k 
...
def find_similar_labels ( 
    ...
    **kwd
    ...
    ----------
    >>> from hlearn.utils import read_data 
    >>> from hlearn.utils.hydroutils import find_similar_labels, classify_k
    >>> data = read_data ('data/boreholes/hf.csv')
    >>> ymap = classify_k(data.k , default_func =True) 
...
def _similarity_rules (lg,  threshold =.5 ):
    ...
    :example:
    >>> from hlearn.utils.hydroutils import _similarity_rules 
...
def _get_y_from_valid_indexes (
    ...
    :example: 
        >>> import numpy as np 
        >>> from hlearn.utils.hydroutils import _get_y_from_valid_indexes 
...
def select_base_stratum (
    ...
    --------
    >>> from hlearn.datasets import load_hlogs 
    >>> from hlearn.utils.hydroutils import select_base_stratum 
...
def get_compressed_vector(
    ...
    -------
    >>> from hlearn.datasets import load_hlogs 
    >>> from hlearn.utils.hydroutils import get_compressed_vector 
    >>> data = load_hlogs().frame # get only the frame  
...
def get_sections_from_depth  (z, z_range, return_index =False ) :
    ...
    --------
    >>> from hlearn.datasets import load_hlogs 
    >>> from hlearn.utils.hydroutils import get_sections_from_depth
    >>> data= load_hlogs().frame  
...
kws: dict, 
    ...
    :func:`~hlearn.utils.hydroutils.get_aquifer_sections`.
...
See Also 
----------
hlearn.utils.hydroutils.get_aquifer_section: compute single section

hlearn.utils.hydroutils.get_aquifer_sections: compute multiple sections 
...
See Also 
----------
hlearn.utils.hydroutils.get_aquifer_sections: 
    compute multiples aquifer sections

Example
-------   
>>> from hlearn.datasets import load_hlogs 
>>> from hlearn.utils.hydroutils import get_aquifer_sections
>>> data = load_hlogs ().frame 
...
def _get_invalid_indexes  ( d, /, valid_indexes, in_arange =False ): 
    ...
    Example 
    -----------
    >>> from hlearn.utils.hydroutils import _get_invalid_indexes
    >>> import numpy as np 
    >>> idx = np.arange (50) 
...
def get_xs_xr_splits (
    ...
    --------
    >>> from hlearn.datasets import load_hlogs 
    >>> from hlearn.utils.hydroutils import get_xs_xr_splits 
    >>> data = load_hlogs ().frame 
...
--------
>>> from hlearn.datasets import load_hlogs
>>> from hlearn.utils.hydroutils import reduce_samples 
...
def is_valid_depth (z, /, zname =None , return_z = False): 
    ...
    --------
    >>> from hlearn.datasets import load_hlogs 
    >>> from hlearn.utils.hydroutils import is_valid_depth 
    >>> d= load_hlogs () 
    >>> X= d.frame 
...
-------
>>> from hlearn.datasets import load_hlogs 
>>> from hlearn.utils.hydroutils import get_aquifer_section 
...
def classify_k (
    ...
    >>> import numpy as np 
    >>> from hlearn.datasets import load_hlogs 
    >>> from hlearn.utils.hydroutils import classify_k 
...
def exportdf (
    ...
    about the `writef` decorator , see :doc:`hlearn.utils.decorator.writef`. 
...
def _cattarget (ar , labels , order=None): 
    """ A shadow function of :func:`hlearn.utils.funcutils.cattarget`. 
...
def validate_labels (t, /, labels, return_bool = False): 
    ...
    >>> from hlearn.utils.mlutils import cattarget, labels_validator 
...
def _name_mxs_labels(*s , sep ='', prefix =""): 
    ...
    Example
    --------
    >>> from hlearn.utils.hydroutils import _name_mxs_labels 
...
def _MXS_if_no(context,  /,  y_true , y_pred , cmsg ='', trailer = "*"): 
    ...
    :example: 
        >>> import numpy as np 
        >>> from hlearn.utils.hydroutils import _MXS_if_no
        >>> y_true = np.arange (5) 
...
def _create_mxs_pseudo_labels(
    ...
    :example: 
        >>> from hlearn.utils.hydroutils import _create_mxs_pseudo_labels 
        >>> import numpy as np 
        >>> y_true = np.arange (5) 
        >>> y_pred = np.arange (1, 6) 

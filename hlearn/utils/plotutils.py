...
def plot_logging ( 
    ...
    **kws
    ...
    >>> from hlearn.utils.plotutils import plot_logging
...
def make_plot_colors(d , / , colors:str | list[str]=None , axis:int = 0, 
    ...
    --------
    >>> import numpy as np 
    >>> from hlearn.utils.plotutils import make_plot_colors
...
def plot_silhouette (X, labels, metric ='euclidean',savefig =None , **kwds ):
    ...
    **kwds : optional keyword parameters
    ...
    See Also
    --------
    hlearn.view.mlplot.plotSilhouette: 
    ...
    ---------
    >>> import numpy as np 
    >>> from hlearn.exlib.sklearn import KMeans 
    >>> from hlearn.datasets import load_iris 
    >>> from hlearn.utils.plotutils import plot_silhouette
    >>> d= load_iris ()
...
def plot_sbs_feature_selection (
    ...
    Parameters 
    ------------
    sbs_estimator : :class:`~.hlearn.base.SequentialBackwardSelection`\
        estimator object
    ...
    sbs_kws: dict, 
        ...
        :class:`~.hlearn.base.SequentialBackwardSelection`
        
    Examples 
    ----------
    (1)-> Plot fitted SBS in action 
    >>> from hlearn.exlib.sklearn import KNeighborsClassifier , train_test_split
    >>> from hlearn.datasets import fetch_data
    >>> from hlearn.base import SequentialBackwardSelection
    >>> from hlearn.utils.plotutils import plot_sbs_feature_selection
...
def plot_regularization_path ( 
    ...
    Examples
    --------
    >>> from hlearn.utils.plotutils import plot_regularization_path 
    >>> from hlearn.datasets import fetch_data
...
def plot_rf_feature_importances (
    ...
    **kws 
    ...
    ---------
    >>> from hlearn.datasets import fetch_data
    >>> from hlearn.exlib.sklearn import RandomForestClassifier 
    >>> from hlearn.utils.plotutils import plot_rf_feature_importances 
...
def plot_confusion_matrix (yt, y_pred, view =True, ax=None, annot=True,  **kws ):
    ...
    >>> from hlearn.datasets import fetch_data
    >>> from hlearn.exlib.sklearn import train_test_split 
    >>> from hlearn.models import pModels 
    >>> from hlearn.utils.plotutils import plot_confusion_matrix
...
def plot_yb_confusion_matrix (
    ...
    >>> from hlearn.exlib.sklearn import train_test_split 
    >>> from hlearn.models import pModels 
    >>> from hlearn.utils.plotutils import plot_yb_confusion_matrix
    >>> # split the  data . Note that fetch_data output X and y 
    >>> X, Xt, y, yt  = train_test_split (* fetch_data ('bagoue analysed'),
                                          test_size =.25  )  
    >>> # train the model with the best estimator 
...
def plot_confusion_matrices (
    ...
    **scorer_kws
    ...
    >>> from hlearn.datasets import fetch_data
    >>> from hlearn.exlib.sklearn import train_test_split 
    >>> from hlearn.models.premodels import p
    >>> from hlearn.utils.plotutils import plot_confusion_matrices 
    >>> # split the  data . Note that fetch_data output X and y 
...
def plot_learning_curves(
    ...
    **kws
    ...
    >>> from hlearn.models.premodels import p 
    >>> from hlearn.datasets import fetch_data 
    >>> from hlearn.utils.plotutils import plot_learning_curves
    ...
    >>> from hlearn.exlib.sklearn import (LogisticRegression, 
                                         RandomForestClassifier, 
...
def plot_naive_dendrogram (
    ...
    :Examples: 
        >>> from hlearn.datasets import fetch_data 
        >>> from hlearn.utils.plotutils import plot_naive_dendrogram
...
def plot_pca_components (
    ...
    :param components: Ndarray, shape (n_components, n_features)or PCA object 
        ...
        :class:`hlearn.analysis.dimensionality.nPCA`. If the object is given 
    ...
    >>> from hlearn.datasets import fetch_data
    >>> from hlearn.utils.plotutils import plot_pca_components
    >>> from hlearn.analysis import nPCA 
...
def plot_clusters (
    ...
    :Example: 
    >>> from hlearn.exlib.sklearn import KMeans, MinMaxScaler
    >>> from hlearn.utils.plotutils import plot_clusters
    >>> from hlearn.datasets import fetch_data 
...
def plot_elbow (
    ...
    ---------
    >>> from hlearn.datasets import load_hlogs 
    >>> from hlearn.utils.plotutils import plot_elbow 
...
def plot_cost_vs_epochs(regs, *,  fig_size = (10 , 4 ), marker ='o', 
    ...
    >>> from hlearn.datasets import load_iris 
    >>> from hlearn.base import AdalineGradientDescent
    >>> from hlearn.utils.plotutils import plot_cost_vs_epochs
...
def plot_mlxtend_heatmap (df, columns =None, savefig=None,  **kws): 
    ...
    >>> from hlearn.datasets import load_hlogs 
    >>> from hlearn.utils.plotutils import plot_mlxtend_heatmap
    >>> h=load_hlogs()
...
def plot_mlxtend_matrix(df, columns =None, fig_size = (10 , 8 ),
    ...
    :example: 
    >>> from hlearn.datasets import load_hlogs 
    >>> from hlearn.utils.plotutils import plot_mlxtend_matrix
    >>> import pandas as pd 
    >>> import numpy as np 
    >>> h=load_hlogs()
...
def make_mpl_properties(n ,prop ='color'): 
    ...
    :Example: 
        >>> from hlearn.utils.plotutils import make_mpl_properties
...
def plotvec2(a,b):
    ...
    *  Write your code below and press Shift+Enter to execute
    ...
    :Example: 
        ...
        >>> import numpy as np 
        >>> from hlearn.utils.plotutils import plotvec2
        >>> a=np.array([1,0])
        >>> b=np.array([0,1])
        >>> Plotvec2(a,b)
...
def get_color_palette (RGB_color_palette): 
    ...
    :Example: 
        ...
        >>> from hlearn.utils.plotutils import get_color_palette 
...
def _skip_log10_columns ( X, column2skip, pattern =None , inplace =True): 
    ...
    :example: 
       >>> from hlearn.datasets import load_hlogs 
       >>> from hlearn.utils.plotutils import _skip_log10_columns 
...
def plot_profiling (
    ...
    **plot_kws
    ...
    Examples 
    ----------
    >>> from hlearn.datasets import make_erp 
    >>> from hlearn.utils.plotutils import plot_profiling 
...
def plot_skew (
    ...
    **kws 
    ...
    Parameters
    -----------
    edi_obj: str, :class:`hlearn.edi.Edi` 
        Full path to edifiles or  :class:`~hlearn.edi.Edi` object. 
    ...
    See Also 
    ---------
    hlearn.methods.em.Processing.skew: 
        Skew equation formulations. 
    hlearn.view.TPlot.plotSkew: 
        Give a consistent plot where user can customize the plot using the 
        plot parameter of :class:`hlearn.property.BasePlot` class.
    ...
    ---------
    >>> import hlearn as wx 
    >>> from hlearn.utils.plotutils import plot_skew 
...
>>> import os 
>>> from hlearn.datasets import fetch_data 
>>> from hlearn.utils.plotutils import plot_strike 
>>> from hlearn.datasets._io import get_data # get edidata in cache  
...
def plot_voronoi(
    ...
    >>> from sklearn.cluster import KMeans 
    >>> from hlearn.utils.plotutils import plot_voronoi
...
def plot_roc_curves (
   ...
   **roc_kws ): 
    ...
    Examples 
    --------
    >>> from hlearn.utils.plotutils import plot_roc_curves 
    >>> from sklearn.datasets import make_moons 
    >>> from hlearn.exlib import ( train_test_split, KNeighborsClassifier, SVC ,

...
from ._pkgs_version import parse as parse_version
...
def _get_threadpool_controller():
    ...
    if not hasattr(hlearn, "_hlearn_threadpool_controller"):
        hlearn._sklearn_threadpool_controller = threadpoolctl.ThreadpoolController()

    return hlearn._sklearn_threadpool_controller
...
_global_config = {
    "assume_finite": bool(os.environ.get("hlearn_ASSUME_FINITE", False)),
    "working_memory": int(os.environ.get("hlearn_WORKING_MEMORY", 1024)),
    "print_changed_only": True,
    "display": "diagram",
    "pairwise_dist_chunk_size": int(
        os.environ.get("hlearn_PAIRWISE_DIST_CHUNK_SIZE", 256)
    ),

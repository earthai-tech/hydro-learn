...
def to_numeric_dtypes (
    ...
    Examples
    ---------
    >>> from hlearn.datasets.dload import load_bagoue
    >>> from hlearn.utils.funcutils import to_numeric_dtypes
...
def listing_items_format ( 
    ...
    Examples
    ---------
    >>> from hlearn.utils.funcutils import listing_items_format 
...
def parse_attrs (attr, /, regex=None ): 
    ...
    Example
    ---------
    >>> from hlearn.utils.funcutils import parse_attrs 
...
def url_checker (url: str , install:bool = False, 
    ...
    Example
    ----------
    >>> from hlearn.utils.funcutils import url_checker 
    >>> url_checker ("http://www.example.com")
    ...  0 # not reacheable 
    >>> url_checker ("https://hlearn.readthedocs.io/en/latest/api/hlearn.html")
    ... 1 
...
def shrunkformat (text: str | Iterable[Any] , 
    ...
    >>> import numpy as np 
    >>> from hlearn.utils.funcutils import shrunkformat
...
def is_installing (
    ...
    Example
    --------
    >>> from hlearn import is_installing
    >>> is_installing(
...
def smart_strobj_recognition(
    ...
    :Example:
        >>> from hlearn.utils.funcutils import smart_strobj_recognition
        >>> from hlearn.methods import ResistivityProfiling 
...
def repr_callable_obj(obj: F  , skip = None ): 
    ...
    >>> from hlearn.utils.funcutils import repr_callable_obj
    >>> from hlearn.methods.electrical import  ResistivityProfiling
...
def accept_types (
    ...
    :Example: 
        ...
        >>> from hlearn.utils.funcutils import accept_types
...
def smart_format(iter_obj, choice ='and'): 
    ...
    :Example: 
        >>> from hlearn.utils.funcutils import smart_format
...
def format_notes(text:str , cover_str: str ='~', inline=70, **kws): 
    ...
    :Example: 
        ...
        >>> from hlearn.utils import funcutils as func 
...
def display_infos(infos, **kws):
    ...
    :Example: 
    >>> from hlearn.utils.funcutils import display_infos
...
def convert_csvdata_from_fr_to_en(csv_fn, pf, destfile = 'pme.en.csv',
    ...
    :Example: 
        ...
        >>> import os 
        >>> import csv 
        >>> from hlearn.utils.funcutils import convert_csvdata_from_fr_to_en
        >>> path_pme_data = r'C:/Users\Administrator\Desktop\__elodata
...
def sanitize_unicode_string (str_) : 
    ...
    :Example:
    >>> from hlearn.utils.funcutils import sanitize_unicode_string 
...
def serialize_data(
    ...
    :Example:
        ...
        >>> import numpy as np
        >>> import hlearn.utils.coreutils import serialize_data
        >>> data = np.arange(15)
...
def load_serialized_data (filename, verbose=0): 
    ...
    :Example:
        ...
        >>> from hlearn.utils.functils import load_serialized_data
...
def find_position_from_sa(
    ...
    :Example:
        ...
        >>> from hlearn.utils.funcutils import find_positon_from_sa
...
def fmt_text(
    ...
    :Example: 
        ...
        >>> from hlearn.utils.funcutils import fmt_text
...
def find_position_bounds( 
    ...
    :Example: 
        ...
        >>> from hlearn.utils.funcutils import find_position_bounds  
...
def reshape(arr , axis = None) :
    ...
    :Example: 
        >>> import numpy as np 
        >>> from hlearn.utils.funcutils import reshape 
...
def ismissing(refarr, arr, fill_value = np.nan, return_index =False): 
    ...
    >>> import numpy as np 
    >>> from hlearn.utils.funcutils import ismissing
...
def make_arr_consistent (
    ...
    ----------
    >>> import numpy as np 
    >>> from hlearn.utils.funcutils import make_arr_consistent
    >>> refarr = np.arange (12) 
    >>> arr = np.arange (7, 10) 
...
def fillNaN(arr, method ='ff'): 
    ...
    >>> import numpy as np 
    >>> from from hlearn.utils.funcutils import fillNaN 
...
def get_params (obj: object 
    ...
    :examples: 
        ...
        >>> from hlearn.utils.funcutils import get_params 
        >>> sigmoid= SVC (
            **{
                'C': 512.0,
                'coef0': 0,
...
def fit_by_ll(ediObjs): 
    ...
    :param ediObjs: list of EDI object, composed of a collection of 
        hlearn.edi.Edi or pycsamt.core.edi.Edi or mtpy.core.edi objects 
    :type ediObjs: hlearn.edi.Edi_Collection 
    ...
    :Example: 
        ...
        >>> from hlearn.utils.funcutils import fit_by_ll
        >>> edipath ='data/edi_ss' 
        >>> cediObjs = EM (edipath) 
...
def make_ids(arr, prefix =None, how ='py', skip=False): 
    ...
    :Example: 
        >>> import numpy as np 
        >>> from hlearn.utils.func_utils import make_ids 
...
def concat_array_from_list (list_of_array , concat_axis = 0) :
    ...
    >>> import numpy as np 
    >>> from hlearn.utils.funcutils import concat_array_from_list 
    >>> np.random.seed(0)
...
def station_id (id_, is_index= 'index', how=None, **kws): 
    ...
    >>> from hlearn.utils.funcutils import station_id 
...
def parse_json(json_fn =None,
    ...
    :Example: 
        ...
        >>> import hlearn.utils.funcutils as FU
...
def is_iterable (
    ...
    :param parse_string: bool, parse string and convert the list of string 
        ...
        :func:`~hlearn.utils.funcutils.str2columns` documentation.
    ...
    :Examples: 
    >>> from hlearn.funcutils.is_iterable 
    ...
    if (parse_string and not transform) and isinstance (y, str): 
        raise ValueError ("Cannot parse the given string. Set 'transform' to"
                          ...
                          " from 'hlearn.utils.funcutils' instead.")
...
def str2columns (text, /, regex=None , pattern = None): 
    ...
    Examples
    ---------
    >>> from hlearn.utils.funcutils import str2columns 
...
def sanitize_frame_cols(
    ...
    Examples 
    ---------
    >>> from hlearn.utils.funcutils import sanitize_frame_cols 
    >>> from hlearn.utils.coreutils import read_data 
...
def to_hdf5(d, /, fn, objname =None, close =True,  **hdf5_kws): 
    ...
    ------------
    >>> import os 
    >>> from hlearn.utils.funcutils import sanitize_frame_cols, to_hdf5 
    >>> from hlearn.utils import read_data 
    >>> data = read_data('data/boreholes/H502.xlsx') 
    >>> sanitize_frame_cols (data, fill_pattern='_', inplace =True ) 
    >>> store_path = os.path.join('hlearn/datasets/data', 'h') # 'h' is the name of the data 
...
def find_by_regex (o , /, pattern,  func = re.match, **kws ):
    ...
    Example
    --------
    >>> from hlearn.utils.funcutils import find_by_regex
    >>> from hlearn.datasets import load_hlogs 
...
def is_in_if (o: iter, /, items: str | iter, error = 'raise', 
    ...
    :example: 
        >>> from hlearn.datasets import load_hlogs 
        >>> from hlearn.utils.funcutils import is_in_if 
        >>> X0, _= load_hlogs (as_frame =True )
...
def map_specific_columns ( 
    ...
    ---------
    >>> from hlearn.datasets import load_hlogs 
    >>> from hlearn.utils.plotutils import map_specific_columns 
    >>> X0, _= load_hlogs (as_frame =True ) 
...
def smart_label_classifier (
    ...
    ----------
    >>> import numpy as np
    >>> from hlearn.utils.funcutils import smart_label_classifier
    >>> sc = np.arange (0, 7, .5 ) 
...
def _smart_mapper (k, /,  kr , return_dict_map =False ) :
    ...
    :Example: 
    >>> from hlearn.utils.funcutils import _smart_mapper 
...
def zip_extractor(
    ...
    Examples 
    ----------
    >>> from hlearn.utils.funcutils import zip_extractor 
    >>> zip_extractor ('hlearn/datasets/data/edis/e.E.zip')
...
def remove_outliers (
    ...
    >>> import numpy as np 
    >>> np.random.seed (42 )
    >>> from hlearn.utils.funcutils import remove_outliers 
...
def normalizer ( arr, /, method ='naive'): 
    ...
    method: str,
       Can be use 'scikit-learn' :class:`~hlearn.exlib.MinMaxScaler` for 
    ...
    ----------
    >>> import numpy as np 
    >>> from hlearn.utils.funcutils import normalizer 
    >>> np.random.seed (42)
...
def _validate_name_in (name, /, defaults = '', expect_name= None, 
    ...
    Examples 
    -------
    >>> from hlearn.utils.funcutils import _validate_name_in 
...
def get_confidence_ratio (
    ...
    array([[13, 16],
           ...
           [10, 19],
           [ 5,  7]])
    >>> from hlearn.utils.funcutils import get_confidence_ratio 
...
def assert_ratio(
    ...
    Examples
    ---------
    >>> from hlearn.utils.funcutils import assert_ratio
    >>> assert_ratio('2')
    2.0
...
def interpolate_grid (
    ...
    ---------
    >>> import numpy as np
    >>> from hlearn.utils.funcutils import interpolate_grid 
...
def random_selector (
    ...
    ----------
    >>> import numpy as np 
    >>> from hlearn.utils.funcutils import random_selector 
    >>> dat= np.arange (42 ) 
...
def get_xy_coordinates (d, / , as_frame = False, drop_xy = False, 
    ...
    ----------
    >>> import hlearn as wx 
    >>> from hlearn.utils.funcutils import get_xy_coordinates 
...
def twinning(
    *d: DataFrame,  
    ...
    ----------
    >>> import hlearn as wx 
    >>> from hlearn.utils.funcutils import twinning 
...
def read_worksheets(*data): 
    ...
    -----------
    >>> import os 
    >>> from hlearn.utils.funcutils import read_worksheets 
    >>> sheet_file= r'F:\repositories\hlearn\data\erp\sheets\gbalo.xlsx'
...
def key_checker (
    ...
    >>> from hlearn.utils.funcutils import key_checker
...
def random_sampling (
    ...
    Examples
    ---------
    >>> from hlearn.utils.funcutils import random_sampling 
    >>> from hlearn.datasets import load_hlogs 
...
def make_obj_consistent_if ( 
    ...
    Examples 
    ----------
    >>> from hlearn.utils.funcutils import make_obj_consistent_if
    >>> from hlearn.exlib import SVC, LogisticRegression, XGBClassifier 
...
def replace_data(
    ...
    ---------
    >>> import numpy as np 
    >>> from hlearn.utils.funcutils import replace_data
...
def convert_value_in (v, /, unit ='m'): 
    ...
    Examples 
    ---------
    >>> from hlearn.utils.funcutils import convert_value_in 
    >>> convert_value_in (20) 
    20.0
...
def split_list(lst:List[Any, ...],/,  val:int, fill_value:Any=None ):
    ...
    Examples
    --------
    >>> from hlearn.utils.funcutils import split_list
    >>> lst = [1, 2, 3, 4, 5, 6, 7, 8]
    >>> val = 3
...
def key_search (
    ...
    Examples
    ---------
    >>> from hlearn.utils.funcutils import key_search 
...
def repeat_item_insertion(text, /, pos, item ='', fill_value=''): 
    ...
    Examples
    ----------
    >>> from hlearn.utils.funcutils import repeat_item_insertion
...
def numstr2dms (
    ...
    **kws
    ...
    Examples
    --------
    >>> from hlearn.utils.funcutils import numstr2dms
...
def storeOrwritehdf5 (
    ...
    **func_kwds, 
    ...
    Examples
    --------
    >>> from hlearn.utils.funcutils import storeOrwritehdf5
    >>> from hlearn.datasets import load_bagoue 
...
def ellipsis2false( *parameters , default_value: Any=False ): 
    ...
    :example: 
        >>> from hlearn.utils.funcutils import ellipsis2false 
        >>> var, = ellipsis2false (...)
        >>> var 
        False

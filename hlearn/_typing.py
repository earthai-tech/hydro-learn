...
""" 
`HLearn`_ Type variables
======================== 

.. |ERP| replace:: Electrical resistivity profiling 

.. _hlearn: https://github.com/WEgeophysics/hlearn/
...
**Array**: Defined for  one dimensional array and `DType` can be specify. For 
    instance, we generated two arrays (`arr1`and `arr2`) for different types:: 
        ...
        >>> import numpy as np
        >>> from hlearn.typing import TypeVar, Array, DType
        >>> T = TypeVar ('T', float) 
...
**NDArray**: Stands for multi-dimensional arrays i.e more than two. Here, the 
    ...
    like this:: 
        ...
        >>> import numpy as np 
        >>> from hlearn.typing import TypeVar, Array, NDarray, DType 
        >>> T =TypeVar ('T', int)
        >>> U = TypeVar ('U')
...
**Sub**: Stands for subset. Indeed, the class is created to define the 
    ...
    is a subset of the whole |ERP| line. The demo is given below:: 
        ...
        >>> import numpy as np 
        >>> from hlearn.typing import TypeVar, DType, Array , Sub
        >>> from hlearn.utils.exmath import _define_conductive_zone
        >>> T= TypeVar ('T', float)
...
**SP**: Stands for Station positions. The unit of position may vary, however, 
    we keep for :mod:`hlearn.method.electrical.ElectricalResistivityProfiling`
    ...
    Here we go: 
        ...
        * Import required modules and generate the whole survey line::
            ...
            >>> import numpy as np 
            >>> from hlearn.typing import TypeVar, DType, SP, Sub 
...
**DataFrame**: Likewise the ``Series`` generic type hint, it stands for 
    ...
    Both can be instanciated as below:: 
        ...
        >>> import numpy as np 
        >>> import pandas pd 
        >>> from hlearn.typing import TypeVar , Any, DType , Series, DataFrame
        >>> T  =TypeVar('T')
...
class Series (DType[T], Generic [T]): 
    ...
    :Example:
        >>> import numpy as np
        >>> import pandas as pd 
        >>> from hlearn.typing import DType, Series  
...
class EDIO(Generic [T]): 
    ...
    EDI object  built from  :class:`hlearn.edi.Edi` or  from `pycsamt`_ 
    or `MTpy`_. 
...
class ZO(Generic [T]): 
    ...
    built from :class:`hlearn.extenals.z.Z` or  :class:`pycsamt.core.z.Z` or 
...
class DataFrame (Series[T], Generic[T]): 
    ...
    :Example:
        ...
        >>> import numpy as np
        >>> import pandas as pd 
        >>> from hlearn.typing import DType, DataFrame 

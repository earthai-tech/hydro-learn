...
class temp2d: 
    ...
    :class:`hlearn.property.BasePlot` parameters. If not given, an 
    ...
    def __getattr__(self, name): 
        msg = ("{0!r} has no attribute {1!r}. Note that {0!r} uses the"
               " plot arguments from `hlearn.property.BasePlot`. Plot arguments"
...
class refAppender (object): 
    ...
    Examples
    ---------
    >>> from hlearn.documentation import __doc__ 
    >>> from hlearn.tools import decorators
    >>> def donothing (): 
                ...
                if self.to =='.csv': 
                    ...
                    self.df.to_excel(self.refout , sheet_name='{0}'.format(
                        self.refout[: int(len(self.refout)/2)]),
                            index=self.writedfIndex) 
            # savepath 
            generatedfile = '_hlearn{}_'.format(
...
@deprecated('Replaced by :class:`hlearn.utils.decorators.catmapflow2`')   
...
class docstring:
    ...
    >>> from hlearn.decorators import writedf , predPlot, docstring 
...
class docAppender: 
    ...
    Examples
    ---------
    >>> from hlearn.decorators import docAppender 
...
class docSanitizer: 
    ...
    Examples
    --------
    >>> from hlearn.decorators import docSanitizer 
    >>> def messdocfunc(): 
